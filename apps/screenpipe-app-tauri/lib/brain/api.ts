...
export async function getBrainStatus(): Promise<BrainStatusDto> {
  const res = await localFetch(`${BASE}/status`);
  return parse<BrainStatusDto>(res, "加载运行状态失败");
}
...
export async function getBrainSource(id: string): Promise<SourceDetailDto> {
