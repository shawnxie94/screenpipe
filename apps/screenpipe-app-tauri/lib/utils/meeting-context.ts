...
export async function fetchActivitySummary(
  ...
  try {
    const res = await localFetch(
      `/activity-summary?start_time=${encodeURIComponent(start)}&end_time=${encodeURIComponent(end)}&include_snippets=false&include_recording=false&include_guidance=false`,
    );
