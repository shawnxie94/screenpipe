...
/**
 * knowledge-section.spec.ts — UI e2e tests for the Knowledge section.
 *
 * Covers:
...
const TEST_SOURCE = "e2e-knowledge-test";
...
/** Navigate away from Knowledge and back to force a component remount + fresh data fetch. */
async function reloadKnowledgeSection() {
  ...
  await waitForTestId("section-knowledge", 10_000);
}

async function selectKnowledgeView(view: "overview" | "memories" | "artifacts") {
  const switcher = await $('[data-testid="knowledge-view-switcher"]');
  await switcher.click();
  const option = await $(`[data-testid="knowledge-filter-${view}"]`);
  await option.waitForDisplayed({ timeout: t(5_000) });
  await option.click();
}

// KNOWN-BROKEN (quarantined): the #4436 Knowledge redesign broke this spec —
...
// spec) is tracked. Re-enable once #4436's knowledge-section regression is fixed.
describe.skip("Knowledge section", function () {
  ...
  before(async () => {
    ...
    const memRes = await fetchJson(`${apiBase}/memories`, {
      ...
      headers: hdrs,
      body: JSON.stringify({
        content: "e2e knowledge test memory - deterministic seed",
        tags: ["e2e-test"],
        importance: 0.5,
        source: TEST_SOURCE,
      }),
    ...
    const tempDir = join(E2E_DATA_DIR, "knowledge-test");
  ...
  it("Knowledge section loads with seeded items", async () => {
    const section = await $('[data-testid="section-knowledge"]');
    ...
    await browser.waitUntil(
      async () => {
        const memItem = await $(
          `[data-testid="knowledge-item-memory-${seededMemoryId}"]`,
        );
    ...
    await saveScreenshot("knowledge-section-loaded");
  });
  ...
  it("switches between memories and artifacts with one dropdown", async () => {
    ...
    const artItem = await $(
      `[data-testid="knowledge-item-artifact-${seededOutputId}"]`,
    );
    expect(await artItem.isExisting()).toBe(false);

    // Switch to artifacts.
    await selectKnowledgeView("artifacts");
    await browser.pause(500);
    ...
    await selectKnowledgeView("memories");
    await browser.pause(500);
    expect(await switcher.getText()).toContain("Memories");
  });

  it("search filters items", async () => {
    const searchInput = await $('[data-testid="knowledge-search-input"]');
  ...
  it("individual artifact delete shows toast", async () => {
    ...
    // Remount the knowledge section to pick up the new artifact
    await reloadKnowledgeSection();

    // Wait for the deletable artifact to appear
    await browser.waitUntil(
      async () => {
        const el = await $(`[data-testid="knowledge-item-artifact-${deleteId}"]`);
    ...
    const deleteBtn = await $(
      `[data-testid="knowledge-delete-artifact-${deleteId}"]`,
    );
    await deleteBtn.click();

    // Confirm in dialog
    const confirmBtn = await $('[data-testid="knowledge-confirm-delete-btn"]');
  ...
  it("individual memory delete shows toast", async () => {
    ...
    // Remount knowledge section to pick up the new memory
    await reloadKnowledgeSection();

    await browser.waitUntil(
      async () => {
        const el = await $(
          `[data-testid="knowledge-item-memory-${deleteMemId}"]`,
        );
        return el.isExisting();
      },
    ...
    // Click delete button
    const deleteBtn = await $(
      `[data-testid="knowledge-delete-memory-${deleteMemId}"]`,
    );
    await deleteBtn.click();
  ...
  it("batch select + delete updates count correctly", async () => {
    ...
    // Remount knowledge section to pick up the new items
    await reloadKnowledgeSection();
    ...
    await browser.waitUntil(
      async () => {
        const el1 = await $(
          `[data-testid="knowledge-item-memory-${batchId1}"]`,
        );
        const el2 = await $(
          `[data-testid="knowledge-item-memory-${batchId2}"]`,
        );
    ...
    // Select both items
    const cb1 = await $(
      `[data-testid="knowledge-checkbox-memory-${batchId1}"]`,
    );
    const cb2 = await $(
      `[data-testid="knowledge-checkbox-memory-${batchId2}"]`,
    );
    await cb1.click();
    await cb2.click();
    ...
    await saveScreenshot("knowledge-after-batch-delete");
  });
  ...
  it("selection count prunes after individual delete", async () => {
    ...
    await browser.waitUntil(
      async () => {
        const el1 = await $(
          `[data-testid="knowledge-item-memory-${pruneMemId}"]`,
        );
        const el2 = await $(
          `[data-testid="knowledge-item-artifact-${pruneArtId}"]`,
        );
        return (await el1.isExisting()) && (await el2.isExisting());
      },
    ...
    // Select both
    const cbMem = await $(
      `[data-testid="knowledge-checkbox-memory-${pruneMemId}"]`,
    );
    const cbArt = await $(
      `[data-testid="knowledge-checkbox-artifact-${pruneArtId}"]`,
    );
    await cbMem.click();
    await cbArt.click();
    await browser.pause(300);
    ...
    const deleteArtBtn = await $(
      `[data-testid="knowledge-delete-artifact-${pruneArtId}"]`,
    );
    ...
    await browser.waitUntil(
      async () => {
        const btn = await $('[data-testid="knowledge-delete-selected"]');
  ...
  it("add new memory", async () => {
    // Click add button
    const addBtn = await $('[data-testid="knowledge-add-memory-btn"]');
    await addBtn.click();

    // Type in textarea
    const textarea = await $('[data-testid="knowledge-add-memory-textarea"]');
    await textarea.waitForExist({ timeout: t(5_000) });
    await textarea.setValue("e2e created memory via add form");

    // Click save
    const saveBtn = await $('[data-testid="knowledge-add-memory-save"]');
    await saveBtn.click();
    ...
    await saveScreenshot("knowledge-after-add-memory");
  });
  ...
  it("artifact markdown preview opens full content in the viewer window", async () => {
    ...
    const preview = await $(
      `[data-testid="knowledge-artifact-preview-${seededOutputId}"]`,
    );
    ...
    await saveScreenshot("knowledge-artifact-viewer");
