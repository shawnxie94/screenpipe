...
/**
 * Local Knowledge entry split smoke coverage.
 *
...
describe("Local Knowledge entry split", function () {
  this.timeout(90_000);
  ...
  it("uses 知识库 as the accessible entry and keeps its three content tabs", async () => {
    const nav = await waitForTestId("nav-knowledge", 15_000);
    await browser.waitUntil(
      async () => (await nav.getText()).includes("知识库"),
      { timeout: t(5_000), timeoutMsg: "Knowledge nav did not use the 知识库 label" },
    );
    await nav.click();
