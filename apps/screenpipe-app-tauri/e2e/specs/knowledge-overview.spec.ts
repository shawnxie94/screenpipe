...
async function openHomeWithDiagnostics() {
  try {
    ...
    await saveScreenshot("knowledge-home-launch-failure").catch(() => "");
    throw error;
  }
}
    ...
    (key: string, eventName: string) => {
      ...
      window.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (new URL(url, window.location.href).pathname === "/api/user") {
          return new Response(
            JSON.stringify({
              id: "e2e-knowledge-canvas-user",
              email: "e2e-knowledge-canvas@screenpipe.test",
              token: "e2e-knowledge-canvas-token",
              app_entitled: true,
...
describe("Knowledge Live Views", function () {
  ...
  it("keeps the delete-last-dashboard template journey scrollable and reviewable", async () => {
    ...
    try {
      const existingViews =
        await invokeOrThrow<KnowledgeView[]>("list_knowledge_views");
      for (const existingView of existingViews) {
        await invokeOrThrow("delete_knowledge_view", { id: existingView.id });
      }

      // A WDIO retry reuses the same app process. Leave Knowledge before opening it
      ...
      const knowledgeNav = await $("[data-testid=nav-knowledge]");
      await knowledgeNav.waitForExist({ timeout: t(10_000) });
      ...
      await browser.waitUntil(
        async () => {
          const preview = await $("[data-testid='knowledge-overview-ai-preview']");
      ...
      const reviewScreenshot = await saveScreenshot(
        "knowledge-template-generated-review",
      );
      ...
      const savedScreenshot = await saveScreenshot(
        "knowledge-template-generated-saved",
      );
      expect(existsSync(savedScreenshot)).toBe(true);
    } finally {
      await piConversation.dispose();
      const remainingViews = await invokeOrThrow<KnowledgeView[]>(
        "list_knowledge_views",
      ).catch(() => []);
      for (const remainingView of remainingViews) {
        await invokeOrThrow("delete_knowledge_view", { id: remainingView.id });
      }
    }
  });
  ...
  it("shows AI-proposed Blocks on an empty Canvas before acceptance", async () => {
    ...
    try {
      ...
      if (existingViews.some((candidate) => candidate.id === emptyViewId)) {
        await invokeOrThrow("delete_knowledge_view", { id: emptyViewId });
      }
      ...
      await browser.pause(100);
      const reviewScreenshot = await saveScreenshot(
        "knowledge-empty-canvas-ai-block-review",
      );
      expect(existsSync(reviewScreenshot)).toBe(true);
    } finally {
      await browser
        ...
        .catch(() => undefined);
      await piConversation.dispose();
      await invokeOrThrow("delete_knowledge_view", { id: emptyViewId }).catch(
        () => undefined,
      );
    }
  });
  ...
  it("reviews Canvas edits per Block without opening Chat", async () => {
    ...
    try {
      ...
      if (existingViews.some((view) => view.id === "process-map")) {
        await invokeOrThrow("delete_knowledge_view", { id: "process-map" });
      }
      await invokeOrThrow("save_knowledge_view", {
        request: {
          id: "process-map",
      ...
      const knowledgeSection = await waitForTestId("section-knowledge", 10_000);
      expect(await knowledgeSection.isDisplayed()).toBe(true);
      ...
      const reviewScreenshot = await saveScreenshot(
        "knowledge-process-map-ai-block-review",
      );
      expect(existsSync(reviewScreenshot)).toBe(true);
      ...
      const screenshot = await saveScreenshot("knowledge-process-map-ai-undo");
  ...
  it("shows requested range and per-block freshness honestly", async () => {
    ...
    if (existingViews.some((view) => view.id === RANGE_TRUTH_VIEW_ID)) {
      await invokeOrThrow("delete_knowledge_view", { id: RANGE_TRUTH_VIEW_ID });
    }
    await invokeOrThrow("save_knowledge_view", {
      request: {
    ...
    const screenshot = await saveScreenshot(
      "knowledge-overview-range-freshness-truth",
    );
    expect(existsSync(screenshot)).toBe(true);
  ...
  it("renders a Pipe-filled Live View template", async () => {
    ...
    for (const viewId of [SELECTABLE_VIEW_ID, FIXED_VIEW_ID]) {
      if (existingViews.some((view) => view.id === viewId)) {
        await invokeOrThrow("delete_knowledge_view", { id: viewId });
      }
    }
...
`,
    ...
    const fixtureViews = await invokeOrThrow<KnowledgeView[]>("list_knowledge_views");
    ...
    // more than a minute for the coherent-update window to elapse. The Knowledge
    ...
    // Force a fresh Knowledge mount after the fixtures and values exist. This also
    ...
    for (const size of SUPPORTED_WINDOW_SIZES) {
      ...
      if (size.label === "minimum") {
        await saveScreenshot("knowledge-overview-minimum-window");
      }
    ...
    const screenshot = await saveScreenshot("knowledge-overview-pipe-filled");
    expect(existsSync(screenshot)).toBe(true);
    ...
    const rangeMenuScreenshot = await saveScreenshot(
      "knowledge-overview-selectable-time-range",
    );
    ...
    const refreshScreenshot = await saveScreenshot(
      "knowledge-overview-refresh-switch-enabled",
    );
    ...
    const fixedScreenshot = await saveScreenshot(
      "knowledge-overview-fixed-range-hidden",
    );
    ...
    await waitForTestId("knowledge-overview-editor", 10_000);
    ...
    const fixedEditorScreenshot = await saveScreenshot(
      "knowledge-overview-fixed-layout-editor",
    );
    ...
    const resizedChartScreenshot = await saveScreenshot(
      "knowledge-overview-canvas-chart-resized",
    );
    ...
    for (const size of [SUPPORTED_WINDOW_SIZES[0], SUPPORTED_WINDOW_SIZES[5]]) {
      ...
      const canvasScreenshot = await saveScreenshot(
        `knowledge-overview-canvas-${size.label}`,
      );
    ...
    await pipesNav.click();
    await waitForTestId("section-pipes", 15_000);
    const restoredKnowledgeNav = await waitForTestId("nav-knowledge", 10_000);
    await restoredKnowledgeNav.click();
    await waitForTestId("section-knowledge", 15_000);
    ...
    const editorScreenshot = await saveScreenshot(
      "knowledge-overview-selectable-layout-editor",
    );
    ...
    await invokeOrThrow("delete_knowledge_view", { id: SELECTABLE_VIEW_ID });
    await invokeOrThrow("save_knowledge_view", { request: viewRequest });
    ...
    await invokeOrThrow("delete_knowledge_view", { id: SELECTABLE_VIEW_ID });
    await invokeOrThrow("delete_knowledge_view", { id: FIXED_VIEW_ID });
  });
});
