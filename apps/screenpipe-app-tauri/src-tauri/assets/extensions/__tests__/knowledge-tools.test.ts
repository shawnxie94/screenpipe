...
// Regression coverage for the Local Knowledge chat tools: execute must ALWAYS
...
describe("knowledge_answer", () => {
  it("returns the trimmed answer payload", async () => {
    ...
    const res = await getTools().knowledge_answer.execute(
      "c1",
      { question: "昨天做了什么" },
...
describe("knowledge_source", () => {
  it("returns the source detail trimmed to the real DTO fields", async () => {
    ...
    const res = await getTools().knowledge_source.execute(
      "c4",
      { source_uid: "src-1" },
      new AbortController().signal,
    );
