...
// First-party Local Knowledge (知迹) tools. These call the engine's own REST API
// directly — no MCP hop — because the chat and the knowledge live in the same
...
export default function (pi: ExtensionAPI) {
  pi.registerTool({
    ...
    label: "知迹知识库问答",
    description:
      "Ask a grounded question over the Local Knowledge (知迹知识库): imported office content (飞书消息、腾讯会议转写), screen/audio work history, and published knowledge. " +
    ...
    async execute(
      ...
      const { ok, status, body } = await callKnowledgeApi("/answer", {
        method: "POST",
      ...
      if (!ok) {
        ...
        return {
          isError: true,
          content: [
            { type: "text" as const, text: `knowledge_answer 失败（${code}）：${message}` },
          ],
        };
      }
  ...
  pi.registerTool({
    ...
    label: "知迹证据原文",
    description:
      "Fetch the full readable text of one Local Knowledge evidence source by its source_uid (the ids cited by knowledge_answer). " +
    ...
    parameters: {
      ...
      properties: {
        source_uid: {
          type: "string",
          description: "The source_uid from a knowledge_answer citation.",
        },
      },
      required: ["source_uid"],
    } as any,
    ...
    async execute(
      ...
      const { ok, status, body } = await callKnowledgeApi(
        `/knowledge/sources/${encodeURIComponent(uid)}`,
        { method: "GET", signal },
      );
      if (!ok) {
        const code = body?.code ?? `http_${status}`;
        ...
        return {
          isError: true,
          content: [
            { type: "text" as const, text: `knowledge_source 失败（${code}）：${message}` },
          ],
        };
      }
      const trimmed = {
