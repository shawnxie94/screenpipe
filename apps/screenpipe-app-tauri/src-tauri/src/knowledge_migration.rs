...
//! (entries + coverage) into the knowledge tables, batch by batch, with id-set
//! and digest validation before the atomic read switch.
//!
//! - While migrating, the activity-history writer is paused (single writer).
//! - After `active`, the knowledge DB is the read source; new entries the writer
...
use crate::activity_history::{knowledge_history_active, migration_in_progress, read_all};
...
pub async fn migrate_activity_history(app: &AppHandle) {
    ...
    match probe_migration_status(app).await {
        MigrationProbe::Active => {
            knowledge_history_active().store(true, Ordering::SeqCst);
            return;
        }
    ...
    if entries_arr.is_empty() && coverage_arr.is_empty() {
        // Mark active so the knowledge starts clean without a second probe.
        let _ = post_json(
            app,
            "/knowledge/migration/activate",
            &json!({
                "entries": {},
                "coverage_spans": 0
            }),
    ...
    if let Err(error) = result {
        tracing::warn!("knowledge history migration incomplete: {error}; will retry next launch");
    }
}
...
async fn probe_migration_status(app: &AppHandle) -> MigrationProbe {
    match get_json(app, "/knowledge/migration/status").await {
...
async fn run_migration(
    ...
    let _ = post_json(app, "/knowledge/migration/begin", &json!({})).await;
    ...
    for (batch_index, chunk) in entries.chunks(100).enumerate() {
        ...
        post_json(app, "/knowledge/migration/import", &payload)
            .await
    ...
    if activation_succeeded(&response) {
        knowledge_history_active().store(true, Ordering::SeqCst);
        tracing::info!("knowledge history migration completed; knowledge enabled");
        Ok(())
    } else {
        Err(format!(
            "validation failed: {}",
...
/// Mirror one new history entry into the knowledge DB (post-activation).
