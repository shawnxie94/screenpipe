...
const mocks = vi.hoisted(() => ({
  ...
  listKnowledgeViewTemplateKits: vi.fn(),
  installKnowledgeViewTemplateKit: vi.fn(),
  saveKnowledgeView: vi.fn(),
  deleteKnowledgeView: vi.fn(),
  loadKnowledgeViewCanvas: vi.fn(),
  saveKnowledgeViewCanvas: vi.fn(),
...
vi.mock("@/lib/utils/tauri", () => ({
  commands: {
    ...
    listKnowledgeViewTemplateKits: mocks.listKnowledgeViewTemplateKits,
    installKnowledgeViewTemplateKit: mocks.installKnowledgeViewTemplateKit,
    saveKnowledgeView: mocks.saveKnowledgeView,
    deleteKnowledgeView: mocks.deleteKnowledgeView,
    loadKnowledgeViewCanvas: mocks.loadKnowledgeViewCanvas,
    saveKnowledgeViewCanvas: mocks.saveKnowledgeViewCanvas,
  },
}));
...
import { KnowledgeOverview, type ViewDefinition } from "../knowledge-overview";
...
beforeEach(() => {
  ...
  mocks.listKnowledgeViewTemplateKits.mockResolvedValue({
    status: "ok",
    data: [],
  });
  mocks.refetchPipes.mockResolvedValue(undefined);
  mocks.showChatWithPrefill.mockResolvedValue(undefined);
  mocks.deleteKnowledgeView.mockResolvedValue({ status: "ok", data: null });
  mocks.loadKnowledgeViewCanvas.mockResolvedValue({ status: "ok", data: null });
  mocks.saveKnowledgeViewCanvas.mockImplementation(async (request) => ({
    status: "ok",
    data: {
...
describe("KnowledgeOverview", () => {
  ...
  it("switches between named dashboards without changing either one", async () => {
    ...
    expect(mocks.saveKnowledgeView).not.toHaveBeenCalled();
  });
  ...
  it("opens AI creation first and keeps blank manual creation available", async () => {
    ...
    await waitFor(() => expect(mocks.saveKnowledgeView).toHaveBeenCalledTimes(1));
    expect(mocks.saveKnowledgeView.mock.calls[0][0]).toEqual(
      expect.objectContaining({
  ...
  it("keeps primary controls visible and moves setup actions into More", async () => {
    ...
    const overviewShell = screen.getByTestId("knowledge-overview-scroll");
  ...
  it("creates a usable starter dashboard on first open", async () => {
    ...
    expect(mocks.saveKnowledgeView).toHaveBeenNthCalledWith(1, {
      id: "my-dashboard",
      title: "我的仪表盘",
      expectedRevision: null,
      timeRange: "today",
    ...
    await waitFor(() => {
      expect(mocks.saveKnowledgeView).toHaveBeenCalledTimes(2);
    });
    const request = mocks.saveKnowledgeView.mock.calls[1][0];
  ...
  it("keeps templates scrollable after deleting the last dashboard", async () => {
    ...
    const emptyState = await screen.findByTestId("knowledge-overview-empty");
    expect(emptyState.className).toContain("min-h-0");
    expect(emptyState.className).toContain("overflow-y-auto");
    expect(
      screen.getByTestId("knowledge-overview-empty-content").className,
  ...
  it("edits the real dashboard, reorders with the keyboard, and resizes without losing its Pipe", async () => {
    ...
    await waitFor(() => expect(mocks.saveKnowledgeView).toHaveBeenCalledTimes(1));
    const request = mocks.saveKnowledgeView.mock.calls[0][0];
  ...
  it("supports pointer drag and drop between dashboard Blocks", async () => {
    ...
    await waitFor(() => expect(mocks.saveKnowledgeView).toHaveBeenCalledTimes(1));
    expect(mocks.saveKnowledgeView.mock.calls[0][0].slots).toEqual([
  ...
  it("persists a declared list-item action and renders its reversible receipt", async () => {
    ...
    mocks.listKnowledgeViews.mockImplementation(async () => ({
      status: "ok" as const,
  ...
  it("stages a Block edit in Canvas and persists only after per-Block acceptance", async () => {
    ...
    await waitFor(() => expect(mocks.saveKnowledgeView).toHaveBeenCalledTimes(2));
    expect(mocks.saveKnowledgeView).toHaveBeenLastCalledWith(
      expect.objectContaining({
        id: "my-overview",
        expectedRevision: 4,
  ...
  it("shows proposed Blocks on an empty Canvas before acceptance", async () => {
    ...
    mocks.listKnowledgeViews.mockResolvedValue({ status: "ok", data: [emptyView] });
    mocks.generateLiveViewWithPi.mockResolvedValue({
  ...
  it("never persists preview-only Block positions while reviewing an added Block", async () => {
    ...
    mocks.saveKnowledgeViewCanvas.mockClear();
    fireEvent.click(screen.getByTestId("canvas-tools-toggle"));
    fireEvent.click(screen.getByTestId("canvas-fit"));

    await waitFor(() =>
      expect(mocks.saveKnowledgeViewCanvas).toHaveBeenCalledTimes(1),
    );
    expect(mocks.saveKnowledgeViewCanvas).toHaveBeenCalledWith(
      expect.objectContaining({
    ...
    expect(
      mocks.saveKnowledgeViewCanvas.mock.calls[0][0].blocks.some(
        (block: { slotId: string }) => block.slotId === "habit-signals",
      ),
    ).toBe(false);
  ...
  it("opens Canvas by default and migrates the removed dashboard mode", async () => {
    ...
    mocks.loadKnowledgeViewCanvas.mockResolvedValue({
      status: "ok",
      data: {
        schema: "live-view-canvas.v1",
        viewId: populatedView.id,
