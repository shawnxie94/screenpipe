...
import {
  commands,
  type AIPreset,
  type KnowledgeViewCanvasDocument,
  type KnowledgeViewComponent,
  type KnowledgeViewDefinition,
  type KnowledgeViewSlot,
  type KnowledgeViewSlotInput,
  type KnowledgeViewTemplateKit,
...
export type ViewComponent = KnowledgeViewComponent;
export type ViewSlot = KnowledgeViewSlot;
export type ViewDefinition = KnowledgeViewDefinition;
...
type PreviewSource = { kind: "template"; kit: KnowledgeViewTemplateKit };
...
function serializedSlots(slots: ViewSlot[]): KnowledgeViewSlotInput[] {
...
function kitSlots(kit: KnowledgeViewTemplateKit): ViewSlot[] {
...
export function KnowledgeOverview({
  ...
  const [canvasDocument, setCanvasDocument] =
    useState<KnowledgeViewCanvasDocument | null>(null);
  const [canvasLoading, setCanvasLoading] = useState(false);
  const [canvasError, setCanvasError] = useState<string | null>(null);
  const [canvasSaving, setCanvasSaving] = useState(false);
  const [aiEditingSlotId, setAiEditingSlotId] = useState<string | null>(null);
  const [templateKits, setTemplateKits] = useState<KnowledgeViewTemplateKit[]>([]);
  ...
  const [proposalTimeRange, setProposalTimeRange] =
    useState<KnowledgeViewTimeRange | null>(null);
  ...
  const canvasLatestRef = useRef<KnowledgeViewCanvasDocument | null>(null);
  ...
  const pendingCanvasSavesRef = useRef(
    new Map<string, KnowledgeViewCanvasDocument>(),
  );
    ...
    const run = async () => {
      ...
      while (pendingCanvasSavesRef.current.size > 0) {
        const nextEntry = pendingCanvasSavesRef.current.entries().next()
          .value as [string, KnowledgeViewCanvasDocument] | undefined;
        if (!nextEntry) break;
        ...
        const result = await commands.saveKnowledgeViewCanvas(request);
  ...
  const changeCanvasDocument = useCallback(
    (next: KnowledgeViewCanvasDocument, { persist }: { persist: boolean }) => {
  ...
  const changeVisibleCanvasDocument = useCallback(
    (next: KnowledgeViewCanvasDocument, options: { persist: boolean }) => {
  ...
  useEffect(() => {
    ...
    setCanvasError(null);
    void commands
      .loadKnowledgeViewCanvas(view.id)
      .then((result) => {
    ...
    async (silent = false, preferredDashboardId?: string | null) => {
      ...
      try {
        ...
        if (loadedViews.length === 0) {
          const starter = await commands.saveKnowledgeView({
          ...
          if (starter.status === "ok") {
            ...
            const retry = await commands.listKnowledgeViews();
  ...
  useEffect(() => {
    let cancelled = false;
    void commands.listKnowledgeViewTemplateKits().then((result) => {
      if (cancelled) return;
      if (result.status === "ok") {
  ...
  const previewTemplate = (kit: KnowledgeViewTemplateKit) => {
    const now = new Date().toISOString();
    setDraft({
  ...
  const runLiveViewBuilderAgentInline = async ({
    ...
    template?: KnowledgeViewTemplateKit | null;
    displayLabel?: string;
  }): Promise<boolean> => {
  ...
  const applyAcceptedAiProposals = async (
    ...
    setSaving(true);
    try {
      const result = await commands.saveKnowledgeView({
        id: view.id,
  ...
  const duplicateDashboard = async () => {
    ...
    try {
      ...
      if (result.status === "error") throw new Error(result.error);
      if (sourceCanvas) {
        const canvasResult = await commands.saveKnowledgeViewCanvas({
          ...toSaveCanvasRequest({
            ...sourceCanvas,
        ...
        if (canvasResult.status === "error") {
          await commands.deleteKnowledgeView(result.data.id);
          throw new Error(
  ...
  const deleteDashboard = async () => {
    ...
    try {
      await pumpCanvasSaves();
      const result = await commands.deleteKnowledgeView(deletingId);
      if (result.status === "error") throw new Error(result.error);
  ...
  const changeTimeRange = async (timeRange: KnowledgeViewTimeRange) => {
  ...
  if (!view && !editing && !aiPreview) {
    return (
      ...
      >
        <div
          data-testid="knowledge-overview-empty-content"
  ...
  if (aiPreview && draft) {
    ...
    return (
      <>
        <div
          data-testid="knowledge-overview-ai-preview"
          ...
          >
            ...
            {view.periodPolicy.type !== "fixed.v1" && (
              <Select
                ...
                onValueChange={(value) =>
                  void changeTimeRange(value as KnowledgeViewTimeRange)
                }
              >
                <SelectTrigger
