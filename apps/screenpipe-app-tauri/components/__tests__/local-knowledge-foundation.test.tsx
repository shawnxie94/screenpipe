...
describe("Local Knowledge frontend contracts", () => {
  ...
  it("keeps knowledge filters in the canonical local endpoint", async () => {
    ...
    expect(mocks.localFetch).toHaveBeenCalledWith(
      "/knowledge/knowledge?state=published&knowledge_type=sop&availability=valid",
    );
  });
  ...
  it("loads work units through the evidence-owned endpoint", async () => {
    ...
    expect(mocks.localFetch).toHaveBeenCalledWith(
      "/knowledge/work-units?scope_key=repo%3Ascreenpipe&limit=20",
    );
  });
