...
### D-12：外部产品的能力迁移后置，先建 zh-local 核心能力

Lumina、Infinitum（以及 zhihuan 的历史设计）的能力迁移排在 zh-local 核心能力之后：先完成通用信封、领域包注册、scope 迁移与放行、Relation 最小契约，并用**现有办公来源路径**做第一验证者（它有真实数据、真实客户隔离需求，且不需要跨仓库改动）。

延后期间：Lumina 与 Infinitum 继续独立运行，各自数据不作为 screenpipe 的事实源；只有显式导入才成为来源，导入按 Connector 契约登记来源命名空间与版本。D-05/D-06 的方向不变，生效时点由 roadmap 的阶段顺序决定。

### D-13：活动层只有一套，摘要必须自足

现状是两套总结逻辑（活动台账 + 活动历史叙事），且证据只留每段首尾两条。改为：

- **唯一活动层**：一次连续活动（沿用活动边界；断开阈值可配，默认 5 分钟）产出「摘要 + 关键字 + 指向原始观测的引用」，存储只落在数据库一处；界面读源改为数据库，现有本地加密存储退为仅导入来源。
- **证据保留按变化**：文本变化才留一条，各来源分别判断；同窗口连续重复只留最早。上限（每个活动）：屏幕文字 120 条、界面操作 240 条、转写不设条数上限（按段落保留）、会议锚点全留、总量兜底 800 条；超限时均匀采样而不是截断尾部；被丢弃的观测要记录条数与原因（没文本 / 墓碑 / 预算）。断开阈值默认 5 分钟，可配置（建议范围 2–30 分钟）。
- **摘要必须自足**：下游第一步就靠它判断这段时间发生了什么，因此不得写“处理了 X 工作”这类空话。长度分档：<15 分钟 80–150 字，15–60 分钟 150–300 字，>60 分钟 300–600 字；必须覆盖正在做的事、推进方式、关键决定与异常、涉及的工具/文件/项目/协作对象、结果与未完成项；关键字 5–12 个（含专名）；每条挂 1–3 条证据。
- **会话计数按活动去重**：同一活动的连续段只算一个会话，避免碎片化把单次观察推成 SOP。
- **旧叙事隔离**：已迁移的历史叙事标 legacy、可查但不参与新抽取输入；重算由用户显式触发。

### D-14：取数由执行体自己做，任务只给指令

现在活动生成把“预加载证据”一次性塞进提示词，并明确禁止执行体调用工具（系统提示却说“只读使用本地 API”，两条自相矛盾）。改为：

- 任务只提供：时间范围、目标、输出契约和铁律；**具体查什么、怎么查由执行体通过只读工具自己完成**。
- 取数方法沉淀为**内部 skill**（查哪些数据、顺序、时间对齐、去重、上限），铁律留在系统指令。理由：取数逻辑随表结构与召回策略演进，需要版本化、复用（活动总结 / WorkUnit 抽取 / 问答都要读同一批数据）和可测试；铁律属于安全与输出契约，必须每次生效。
- 可复现要求：记录查询轨迹并把查询结果指纹纳入输入指纹；只读不得写库；设查询上限；输出必须带证据引用。

### D-15：WorkUnit 从活动层抽取，知识只从 WorkUnit

- WorkUnit 抽取**先读活动摘要与关键字**，必要时按该活动的时间范围 + 关键字召回原始观测补细节；不再无条件重读原始材料。
- WorkUnit 内容要尽量丰富：现有字段（任务/输入/动作/决定/异常/输出/结果）保持，新增「流程」（有序步骤）、「环境」（工具/文件/项目/协作对象）、「细节」；新字段同样强制证据引用。属 schema v2，排在本批最后。
- 知识只从 WorkUnit 提炼（不再直接读原始证据），现有 3 会话门槛与单次观察标注规则保留。

### D-16：内部 skill 用中性真源，按 Agent 适配注入

现行仓库把内置 skill 的打包来源放在 `.claude/skills/`，运行时的技能扫描根也只挂了 `~/.claude/skills`。这会让内部能力隐性强依赖某一个 Agent。改为：

- **真源中性**：skill 正文只存一处（随应用交付的内置资源），运行时落到应用自己的 skill store（`<data_dir>/skills`，已有保留名单与来源清单）；正文里不出现任何 Agent 品牌。
- **按 Agent 注入**：由现有 Agent 适配层投递——`crates/screenpipe-engine/src/cli/agent.rs` 的 `AgentLayout` 表已按 Agent 声明 `skills_dir` / `mcp_path` / `mcp_format`（OpenClaw、Hermes、Claude Code、Claude Desktop、Codex、Gemini CLI、Cursor…），`crates/screenpipe-core/src/agents/` 负责 native Pi 与 ACP 的指令注入（AGENTS.md / CLAUDE.md / 首轮上下文）。新增运行时只加一行布局，业务代码不写死路径。
- **仓库路径改名**：内置 skill 的仓内来源目录改为中性名（如 `skills/`），Claude Code 需要的 `.claude/skills` 入口用链接或构建复制保留；技能扫描根按 `AgentLayout` 生成，不再写死单个目录。
- **版本进指纹**：skill 正文哈希与提示词版本同等参与输入指纹（skill 内容有现成哈希机制）。
- 共享取数 skill 与活动总结 / 工作单元 / 知识提炼三个任务 skill **全部实现**，不按成本分阶段。
...
| 必须先冻结 | 依赖 | 触发时点 |
|---|---|---|
| 对象信封 + scope 语义 | D-11（已决定：同库逻辑隔离） | 接入任何新来源类型或第二个领域包之前（首个验证者用现有办公来源路径） |
| 活动层 schema、摘要口径、取数契约 | D-13 / D-14 / O-07 / O-11 | 改造实现前（P1 收尾批内） |
| skill 存放与按 Agent 注入契约 | D-16 | 改造实现前 |
...
| O-07 | 活动断开阈值与各来源的证据保留上限取值 | 已收口 | 按建议定下（2026-09-10）：断开阈值默认 5 分钟可配（建议 2–30 分钟）；每活动上限 屏幕 120 / 界面 240 / 转写不限 / 会议全留 / 总量 800，超限均匀采样；丢弃条数与原因必记录。用一天真实数据复核这两个取值 |
| O-08 | 关键字是否进入检索（D-13 的第二步） | 摘要质量稳定后 | 保留集验证：关键字能召回正确活动，且不引入越范围内容 |
| O-09 | 旧叙事与新活动层的共存策略（是否重算、何时重算） | 改造实现前 | 旧叙事不出现在新抽取输入；重算按用户显式触发执行 |
| O-10 | WorkUnit schema v2 的字段与审核界面呈现 | 该批最后一步之前 | 新增字段可审核、可引用，旧版本仍可读 |
| O-11 | 取数 skill 的查询上限与可复现验证方式 | 改造实现前 | 同一窗口重跑两次，输出指纹一致或差异可解释 |
| O-12 | 代码与数据库的新名 | 已收口 | 已定（2026-09-10）：短名 `knowledge`（模块、路由 `/knowledge/*`、表 `knowledge_*`、任务定义 `knowledge.*`、类型 `Knowledge*`）。判定见改名批次验收：全仓无 `brain` 领域标识残留（已应用迁移文件除外）、存量库靠新增迁移平滑升级、现有测试套件全绿 |
...
- 不在核心能力建成前迁移 Lumina/Infinitum 的采集与发布能力（见 D-12）；延后期间两者继续独立运行，其数据不作为 screenpipe 事实源。
...
| 2026-09-10 | 1.4 | 用户指出外部产品能力迁移应后置：新增 D-12（先建 zh-local 核心能力，首个验证者用现有办公来源路径；延后期间 Lumina/Infinitum 继续独立运行、不入事实链）；§5 信封冻结触发条件改为“接入任何新来源类型或第二个领域包之前”，不再绑定 reading/radar；§7 补对应排除项 | 阶段顺序由 roadmap 依 D-12 调整（Lumina 直采由 P2 后移）；实现状态未变（still not_started） |
