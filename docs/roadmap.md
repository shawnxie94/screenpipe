...
- **沉淀管线修复（切分与投递优化，进行中）**：计划 `docs/plans/knowledge-pipeline-cadence-execution-plan.md`。R1（正确性与切分）已实现并**通过真实副本验证**：失败即终态不自动重试、错误消息落库与 API 透出、发现预筛；锚点/合并不变量/单活跃版本与显式回退；发现与摘要的输入指纹改用稳定 interval_key，重复 reconcile 后同窗口**新增投递 0**（首轮 90），副本不变量全绿（不重叠/证据归属/孤儿 0/单活跃版本）。已知未收项：v1 存量 69 条证据时间越界（35 条边界口径、34 条实质越界）未修；真实窗口 112/112 相邻边界为跨身份切换，碎片目标已证伪 §4.4 可达、移交工作项层（Plan B）。R1 未启动真机桌面、未接桌面 tick。R2（四层节拍配置化、知识三机制投递（模型提名+变更驱动+冷却期）、legacy 自动叙事下线、设置页与 UI 展示）仍待执行。此前“P1 收尾遗留项全部落地”仅指 2026-09-11 那批合成验收，不代表沉淀管线无缺陷；dev 冒烟发现的重复投递与额度消耗问题由本计划收口。
