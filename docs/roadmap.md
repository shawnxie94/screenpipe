...
- **沉淀管线修复（切分与投递优化，进行中）**：计划 `docs/plans/knowledge-pipeline-cadence-execution-plan.md`。R1（正确性与切分）已实现并通过独立验收（失败即终态不自动重试、抽取指纹含成员活动、错误消息落库与 API 透出、发现预筛；锚点/合并不变量/单活跃版本与显式回退）；R1 未启动真机、未接桌面 tick。R2（四层节拍配置化、知识三机制投递（模型提名+变更驱动+冷却期）、legacy 自动叙事下线、设置页与 UI 展示）仍待执行。此前“P1 收尾遗留项全部落地”仅指 2026-09-11 那批合成验收，不代表沉淀管线无缺陷；dev 冒烟发现的重复投递与额度消耗问题由本计划收口。
