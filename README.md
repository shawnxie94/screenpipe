...
统一产品与迭代入口见 SQLite Roadmap Artifact：当前重点是移除历史知识域、把办公接入解耦为独立 Connector，然后扩展接入渠道并深化本地检索。screenpipe 只保留采集、接入、存储与检索能力，不内建记忆、知识库、研究综合或行动闭环。
...
- 新需求先记录到 SQLite Roadmap Artifact，再拆成可验证的实现步骤；不要用一次性脚本或临时文档代替长期设计。
- 测试和提交前检查见 [AGENTS.md](AGENTS.md)、[CONTRIBUTING.md](CONTRIBUTING.md) 和 [TESTING.md](TESTING.md)。

## 文档地图

- Roadmap：SQLite Roadmap Artifact 记录当前产品方向、进行中事项与后续阶段。
