 ...
 * npm's `screenpipe-mcp@0.18.15` was built from a tree that drifted from the
 * repo — and because nobody bumped `package.json`, the registry's "0.18.15"
 * and the repo's "0.18.15" were two different trees with one name (SCR-352). The version string is the only thing support can use to tell
