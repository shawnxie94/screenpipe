...
impl DatabaseManager {
    ...
    pub async fn knowledge_enable(&self) -> Result<String, SqlxError> {
        let mut tx = self.begin_immediate_with_retry().await?;
    ...
    pub async fn knowledge_bump_source_epoch(&self) -> Result<(), SqlxError> {
        let mut tx = self.begin_immediate_with_retry().await?;
    ...
    pub async fn knowledge_current_deletion_epoch(&self) -> Result<i64, SqlxError> {
        Ok(self.knowledge_get_state().await?.deletion_epoch)
    }

    pub async fn knowledge_bump_publication_epoch(&self) -> Result<(), SqlxError> {
        let mut tx = self.begin_immediate_with_retry().await?;
    ...
    pub async fn knowledge_bump_change_epoch(&self) -> Result<(), SqlxError> {
        let mut tx = self.begin_immediate_with_retry().await?;
