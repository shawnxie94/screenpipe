...
fn frame_input(table: &str, id: i64, revision: &str) -> super::sources::BrainSourceInput {
    super::sources::BrainSourceInput {
...
async fn office_identity_is_provider_scoped() {
    ...
    let input = super::sources::BrainSourceInput {
    ...
    let a = db.brain_register_source(&input).await.unwrap();
    assert!(a.created);
    ...
    let b = db.brain_register_source(&other_acc).await.unwrap();
    assert!(b.created);
    assert_ne!(a.source_uid, b.source_uid);
    // Same object again dedupes.
    let c = db.brain_register_source(&input).await.unwrap();
...
async fn job_lifecycle_dedupe_claim_and_token_guard() {
    ...
    let second = db
        .brain_claim_next_job(&[super::types::BrainJobKind::Extract], "worker-b", 30_000)
        .await
        .unwrap();
...
async fn transient_failure_requeues_then_exhausts() {
    ...
    assert!(db.brain_retry_job(id).await.unwrap());
    let again = db.brain_get_job(id).await.unwrap().unwrap();
...
async fn expired_lease_is_reaped_with_attempts_preserved() {
    ...
    let (id, _) = db
        ...
        .unwrap();
    let claimed = db
        .brain_claim_next_job(&[super::types::BrainJobKind::Answer], "w", 30_000)
        .await
        .unwrap()
        .unwrap();
    ...
    sqlx::query("UPDATE brain_jobs SET lease_expires_at = '2000-01-01T00:00:00Z' WHERE id = ?1")
        ...
        .await
        .unwrap();
    let reaped = db.brain_reap_expired_leases().await.unwrap();
    assert_eq!(reaped, 1);
    let job = db.brain_get_job(id).await.unwrap().unwrap();
