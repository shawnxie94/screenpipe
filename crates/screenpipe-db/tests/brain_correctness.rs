...
// Review-derived Local Brain persistence probes. Synthetic SQLite only.

use std::sync::Arc;

use chrono::{Duration, Utc};
use screenpipe_db::{
    compute_input_hash, BrainJobKind, BrainSearchDocInput, BrainSourceInput, DatabaseManager,
...
fn frame_input(table: &str, id: i64, revision: &str) -> BrainSourceInput {
...
async fn real_frame_input(db: &Arc<DatabaseManager>, revision: &str) -> BrainSourceInput {
...
async fn migration_db_source_registration_is_idempotent_and_revision_aware() {
    ...
    assert_eq!(
        db.brain_get_source(&first.source_uid)
            .await
            .unwrap()
            .unwrap()
            .revision,
...
async fn search_compatibility_projection_upsert_query_and_delete() {
    ...
    db.brain_search_upsert_doc(&input).await.unwrap();

    let first = db
        .brain_search_topk("compatmarker", &["source"], 10)
        .await
        .unwrap();
    ...
    assert!(db
        .brain_search_topk("compatmarker", &["knowledge"], 10)
        .await
        .unwrap()
        .is_empty());

    let updated = BrainSearchDocInput {
        ref_revision: "rev-b".into(),
        body: "compatmarker updated revision".into(),
        ..input.clone()
    };
    db.brain_search_upsert_doc(&updated).await.unwrap();
    let second = db
        .brain_search_topk("compatmarker", &["source"], 10)
        .await
        .unwrap();
    ...
    db.brain_search_delete_doc(&updated.doc_id).await.unwrap();
    assert!(db
        .brain_search_topk("compatmarker", &["source"], 10)
        .await
        .unwrap()
...
async fn deletion_db_user_erase_tombstones_source_and_blocks_reimport() {
    ...
    db.brain_delete_source(&registration.source_uid, true)
        .await
        .unwrap();
    assert!(db
        .brain_get_source(&registration.source_uid)
        .await
        .unwrap()
        .is_none());
...
async fn jobs_db_job_lifecycle_deduplicates_claims_and_guards_tokens() {
    ...
    let claimed = db
        .brain_claim_next_job(&[BrainJobKind::Extract], "worker-a", 30_000)
        .await
        .unwrap()
        .unwrap();
    assert_eq!(claimed.id, first_id);
    assert!(db
        .brain_claim_next_job(&[BrainJobKind::Extract], "worker-b", 30_000)
        .await
        .unwrap()
        .is_none());
    assert!(!db
        .brain_complete_job(first_id, "stale-token", None, None)
        .await
        .unwrap());
    assert!(db
        .brain_complete_job(
        ...
        .unwrap());
    assert_eq!(
        db.brain_get_job(first_id).await.unwrap().unwrap().state,
        "succeeded"
    );
}
...
async fn jobs_db_expired_lease_reaps_while_preserving_model_call_count() {
    let db = db().await;
    let (job_id, _) = db
        .brain_enqueue_job(BrainJobKind::Answer, None, None, None, None, None)
        .await
        .unwrap();
    let claimed = db
        .brain_claim_next_job(&[BrainJobKind::Answer], "worker", 30_000)
        .await
        .unwrap()
        .unwrap();
    assert!(db
        .brain_job_model_call(job_id, &claimed.lease_token)
        .await
        .unwrap());
    let mut tx = db.begin_immediate_with_retry().await.unwrap();
    sqlx::query("UPDATE brain_jobs SET lease_expires_at = '2000-01-01T00:00:00Z' WHERE id = ?")
        ...
        .unwrap();
    tx.commit().await.unwrap();
    assert_eq!(db.brain_reap_expired_leases().await.unwrap(), 1);
    let row = db.brain_get_job(job_id).await.unwrap().unwrap();
    assert_eq!(row.state, "pending");
    assert_eq!(row.model_calls, 1, "call counters never reset");
    assert!(!db
        .brain_complete_job(job_id, &claimed.lease_token, None, None)
        .await
        .unwrap());
}
...
async fn deletion_db_deletion_barrier_cancels_jobs_and_invalidates_consumers() {
    let db = db().await;
    let registration = db
        .brain_register_source(&real_frame_input(&db, "rev-a").await)
        .await
        .unwrap();
    let work_unit = db
    ...
    let consumers = db
        .brain_invalidate_consumers(&[registration.source_uid])
        .await
        .unwrap();
    assert!(consumers
        .iter()
    ...
    let knowledge_row = db.brain_get_version(&knowledge, 1).await.unwrap().unwrap();
    assert_eq!(
        knowledge_row.body,
        r#"{"body":"deletion fixture knowledge"}"#
    );
...
async fn publication_db_retention_preserves_published_excerpt_before_raw_delete() {
    ...
    let registration = db
        .brain_register_source(&real_frame_input(&db, "retention-rev").await)
        .await
        .unwrap();
    let frame_id = db
        .brain_get_source(&registration.source_uid)
    ...
    .await
    .unwrap();
    let version = db.brain_get_version(&knowledge, 1).await.unwrap().unwrap();
    assert!(db
    ...
    let text = db
        .brain_source_text(&registration.source_uid, "retention-rev")
        .await
        .unwrap()
...
async fn publication_db_publish_lifts_feedback_pause_so_the_correction_loop_recloses() {
    ...
    let version = db.brain_get_version(&knowledge, 1).await.unwrap().unwrap();
    assert!(db
        .brain_publish_version(&knowledge, 1, &version.revision, None, "2099-01-01T00:00:00Z")
        .await
        .unwrap());
    ...
    db.brain_set_paused(&knowledge, true).await.unwrap();
    ...
    let candidate = db
        ...
        .await
        .unwrap();
    let candidate_row = db.brain_get_version(&knowledge, candidate).await.unwrap().unwrap();
    assert!(db
        .brain_publish_version(
            &knowledge,
            candidate,
