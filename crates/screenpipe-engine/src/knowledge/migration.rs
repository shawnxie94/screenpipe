...
//! History preservation migration (activity-history store → knowledge tables)
...
use screenpipe_db::{KnowledgeHistoryBatch, KnowledgeHistoryCoverageInput, KnowledgeHistoryEntryInput, KnowledgeHistoryEntryRow};
...
async fn history_range(
    ...
    match state.db.knowledge_history_entries_between(&start, &end).await {
...
/// entry into the knowledge DB (single durable truth after the switch).
async fn history_entry_upsert(
    ...
    match state
        .db
        .knowledge_history_upsert_entry(
            &entry.id,
            &entry.body,
            &entry.encoding,
...
async fn migration_status(State(state): State<Arc<AppState>>) -> Response {
    let (phase, cursor, validated) = match state.db.knowledge_history_migration_state().await {
        Ok(v) => v,
        Err(e) => return err_response(KnowledgeError::new("db_error", e.to_string(), true)),
    };
    let (entries, spans) = state
        .db
        .knowledge_history_counts()
        .await
        .unwrap_or((0, 0));
    Json(json!({
        "phase": phase,
...
async fn migration_begin(State(state): State<Arc<AppState>>) -> Response {
    match state
        .db
        .knowledge_history_set_phase("importing", None)
        .await
    {
        Ok(()) => Json(json!({"ok": true})).into_response(),
        Err(e) => err_response(KnowledgeError::new("db_error", e.to_string(), true)),
...
async fn migration_import(
    State(state): State<Arc<AppState>>,
    Json(batch): Json<KnowledgeHistoryBatch>,
) -> Response {
    ...
    match state.db.knowledge_history_import_batch(&batch).await {
...
async fn migration_activate(
    ...
    let local = match db.knowledge_history_digests().await {
        Ok(rows) => rows,
        Err(e) => return err_response(KnowledgeError::new("db_error", e.to_string(), true)),
    };
    ...
    for (id, body_text) in local {
        ...
        hasher.update(knowledge_history_canonical(&body_text));
    ...
    let digest_mismatches: Vec<&String> = body
        ...
        .map(|(id, _)| id)
        .collect();
    let (spans_local, _) = db.knowledge_history_counts().await.unwrap_or((0, 0));
        ...
        || spans_local != body.coverage_spans as i64
    {
        ...
        let _ = db
            .knowledge_history_set_phase("importing", Some(&message.to_string()))
            .await;
        return err_response(KnowledgeError::new(
            "validation_failed",
            message.to_string(),
    ...
    match db.knowledge_history_activate().await {
        Ok(()) => {
            // Enabling the knowledge for the first time freezes the boundary.
            let _ = db.knowledge_enable().await;
...
fn knowledge_history_canonical(body: &str) -> Vec<u8> {
...
async fn create_backfill(
    ...
    if start >= end {
        return err_response(KnowledgeError::new("bad_range", "回填范围不合法", false));
    }
