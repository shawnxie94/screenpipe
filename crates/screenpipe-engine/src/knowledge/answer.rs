...
use super::executor::KnowledgeLimits;
use super::prompts;
use super::routes::err_response;
use super::search::{self, KnowledgeSearchArgs};
use super::types::{
    AnswerRequest, AnswerResponse, AnswerStatus, KnowledgeError, ClaimDto, EvidenceRefDto,
    RetrievalReportDto, RouteStatus,
};
use crate::server::AppState;

/// `POST /answer` is a top-level route (TRD §6.2) — mounted at the server
/// root, not under the `/knowledge` nest.
...
async fn answer(State(state): State<Arc<AppState>>, Json(req): Json<AnswerRequest>) -> Response {
    ...
    let _permit = match tokio::time::timeout(
        KnowledgeLimits::ANSWER_QUEUE_DEADLINE,
        ANSWER_LANE.acquire(),
    )
    .await
    {
        Ok(Ok(permit)) => permit,
        _ => {
            return err_response(KnowledgeError::busy(
                (KnowledgeLimits::ANSWER_QUEUE_DEADLINE.as_millis() as u64).max(500),
            ));
        }
    };

    let guard = state.knowledge.barrier.begin_emission();
    let result = run_answer(&state.knowledge.db, &req, started).await;
    if !guard.valid() {
...
async fn run_answer(
...
) -> Result<AnswerResponse, KnowledgeError> {
    ...
    let result = search::search(KnowledgeSearchArgs {
        db,
    ...
    let Some(shared) = super::shared().map(|s| s.clone()) else {
        return Err(KnowledgeError::new("desktop_unavailable", "Knowledge 服务不可用", false));
    };
    ...
    let budgeted = super::executor::BudgetedExecutor::new(executor, KnowledgeLimits::MAX_MODEL_CALLS);
    ...
    let completion = tokio::time::timeout(
        KnowledgeLimits::ANSWER_TOTAL_DEADLINE.saturating_sub(
    ...
    let raw = match completion {
        ...
        Err(_) => {
            cancel.cancel();
            return Err(KnowledgeError::new("deadline", "回答总时限已超", true));
        }
    };

    let parsed = super::extract::parse_json_object(&raw).ok_or_else(|| {
        KnowledgeError::new("invalid_output", "模型输出不是合法 JSON", true)
    })?;
...
async fn answer_response(
    ...
    // Persist only on explicit save (POST /knowledge/answers/:id/save); the row
