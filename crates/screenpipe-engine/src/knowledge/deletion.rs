...
use super::types::{KnowledgeError, SourceKind};
...
fn validate_scope(value: &serde_json::Value) -> Result<(DeletionScope, Vec<String>), KnowledgeError> {
    let scope: DeletionScope = serde_json::from_value(value.clone())
        .map_err(|e| KnowledgeError::new("deletion_scope_invalid", e.to_string(), true))?;
    let uids = scope.uids.clone().ok_or_else(||
        KnowledgeError::new("deletion_scope_invalid", "deletion scope is missing uids", true))?;
    if !matches!(scope.kind.as_str(), "sources" | "retention_range") {
        return Err(KnowledgeError::new("deletion_scope_invalid", "unknown deletion scope kind", true));
    }
    if uids.iter().any(|uid| uid.trim().is_empty()) {
        return Err(KnowledgeError::new("deletion_scope_invalid", "deletion scope contains empty uid", true));
    }
    Ok((scope, uids))
}
...
impl DeletionService {
    fn journal_path(&self) -> PathBuf {
        self.knowledge_dir.join("deletion-journal.jsonl")
    }
    ...
    async fn journal_entries(&self) -> Result<Vec<JournalEntry>, KnowledgeError> {
        let content = match tokio::fs::read_to_string(self.journal_path()).await {
            Ok(c) => c,
            Err(e) if e.kind() == std::io::ErrorKind::NotFound => return Ok(Vec::new()),
            Err(e) => return Err(KnowledgeError::new("journal_read_failed", e.to_string(), true)),
        };
        let entries: Vec<JournalEntry> = content.lines().enumerate().filter(|(_, line)| !line.trim().is_empty()).map(|(n, line)| {
            serde_json::from_str::<JournalEntry>(line).map_err(|e| KnowledgeError::new(
                "journal_corrupt", format!("deletion journal line {} is invalid: {}", n + 1, e), true))
        }).collect::<Result<_, _>>()?;
        let mut previous = 0;
        for entry in &entries {
            if entry.seq <= previous || entry.seq <= 0 {
                return Err(KnowledgeError::new("journal_corrupt", "journal seq must be positive and strictly increasing", true));
            }
            previous = entry.seq;
        }
        Ok(entries)
    ...
    async fn journal_max_seq(&self) -> Result<i64, KnowledgeError> {
    ...
    pub async fn delete_sources(
        ...
        cause: DeletionCause,
    ) -> Result<DeletionOutcome, KnowledgeError> {
    ...
    async fn delete_sources_scoped(
        ...
        self.journal_append(journal_seq, cause, &scope)
            .await
            .map_err(|e| KnowledgeError::new("journal_write_failed", e.to_string(), true))?;

        // 2. Record + barrier + job cancellation.
        let deletion_id = self
            .db
            .knowledge_record_deletion_with_seq(cause, &scope.to_string(), journal_seq)
            .await
            .map_err(db_err)?;
        drop(_journal_lock);
        self.db
            .knowledge_activate_deletion_barrier(deletion_id, true)
            .await
            .map_err(db_err)?;
        ...
        let knowledge_hits = self
            .db
            .knowledge_invalidate_consumers(&source_uids)
            .await
            .map_err(db_err)?;
        ...
        let hit_tuples: Vec<(String, String, Option<String>)> = knowledge_hits
            ...
            .collect();
        super::items::apply_knowledge_hits(&self.db, &hit_tuples, cause).await;
        ...
        for uid in &source_uids {
            match self.db.knowledge_delete_source(uid, cause.is_user_initiated()).await {
                Ok(()) => removed += 1,
                Err(e) => {
                    ...
                    let _ = self
                        .db
                        .knowledge_add_cleanup_item(deletion_id, "db_body", uid)
                        .await;
                    failed = true;
                    tracing::warn!("knowledge deletion: body removal pending for {}: {}", uid, e);
                }
            }
        }
        self.db
            .knowledge_update_deletion_state(deletion_id, if failed { "failed" } else { "completed" })
            .await
            .map_err(db_err)?;
        if !failed {
    ...
    pub async fn delete_memory(&self, memory_id: i64) -> Result<DeletionOutcome, KnowledgeError> {
        let uids = self
            .db
            .knowledge_source_uids_for_locator("memories", memory_id)
            .await
            .map_err(db_err)?;
    ...
    pub async fn propagate_retention_range(
        ...
        let uids = self
            .db
            .knowledge_source_uids_captured_between(
                &[
            ...
            .map_err(db_err)?;
        if media_only {
            self.db.knowledge_set_source_media(&uids, false).await.map_err(db_err)?;
            return Ok(DeletionOutcome {
                deletion_id: 0,
                journal_seq: 0,
                sources_removed: 0,
        ...
        let (protected, unprotected) = self
            .db
            .knowledge_split_published_protected(&uids)
            .await
            .map_err(db_err)?;
        if !protected.is_empty() {
            self.db
                .knowledge_set_source_state(&protected, "archived")
                .await
                .map_err(db_err)?;
            self.db.knowledge_set_source_media(&protected, false).await.map_err(db_err)?;
        }
        let scope = json!({
    ...
    pub async fn recover_incomplete(&self) -> Result<usize, KnowledgeError> {
        let _wave_lock = self.wave_lock.lock().await;
        let mut recovered = 0;
        ...
        for entry in &entries {
            ...
            if let Some(existing) = self.db.knowledge_get_deletion_by_journal_seq(entry.seq).await.map_err(db_err)? {
                ...
                if existing.cause != entry.cause.as_str() || existing_scope != entry.scope {
                    return Err(KnowledgeError::new("journal_identity_mismatch", format!("journal seq {} does not match DB wave", entry.seq), true));
                }
            } else {
                let _lock = self.journal_lock.lock().await;
                self.db.knowledge_record_deletion_with_seq(
                    entry.cause,
        ...
        let incomplete = self
            .db
            .knowledge_incomplete_deletions()
            .await
            .map_err(db_err)?;
        ...
        for wave in incomplete {
            if wave.state == "journal_only" {
                ...
                self
                    .db
                    .knowledge_activate_deletion_barrier(wave.id, true)
                    .await
                    .map_err(db_err)?;
            }
            ...
            let mut failed = false;
            for uid in &uids {
                if let Err(e) = self.db.knowledge_delete_source(uid, wave.cause == DeletionCause::UserErase.as_str()).await {
                    failed = true;
                    self.db
                        .knowledge_add_cleanup_item(wave.id, "db_body", uid)
                        .await
                        .map_err(db_err)?;
                    tracing::warn!("knowledge recovery: source {} remains pending: {}", uid, e);
                }
            }
            ...
            let pending = self.db.knowledge_pending_cleanup_items(wave.id).await.map_err(db_err)?;
            for (item_id, exit, target, _state, _attempts) in pending {
                let retry = if exit == "db_body" {
                    self.db
                        .knowledge_delete_source(&target, wave.cause == DeletionCause::UserErase.as_str())
                        .await
                ...
                match retry {
                    Ok(()) => self.db.knowledge_cleanup_item_done(item_id).await.map_err(db_err)?,
                    Err(error) => {
                        failed = true;
                        self.db.knowledge_cleanup_item_failed(item_id, &error).await.map_err(db_err)?;
                    }
                }
            }
            self.db
                .knowledge_update_deletion_state(wave.id, if failed { "failed" } else { "completed" })
                .await
                .map_err(db_err)?;
            failed_any |= failed;
            recovered += 1;
        }
        let _ = self.db.knowledge_prune_terminal_jobs(500).await;
        if failed_any {
            return Err(KnowledgeError::new(
...
fn db_err(e: sqlx::Error) -> KnowledgeError {
    KnowledgeError::new("db_error", e.to_string(), true)
}
