...
async fn control(
    ...
    let svc = service(&state);
    let result: Result<u64, KnowledgeError> = match body.action.as_str() {
        "pause" => svc
            .db
            .knowledge_cancel_jobs_for_scope(
                screenpipe_db::KnowledgeJobKind::OfficeSync,
                &provider,
                "paused_by_user",
            )
            .await
        ...
        _ => {
            return err_response(KnowledgeError::new("bad_request", "未知操作", false));
        }
    };
    match result {
