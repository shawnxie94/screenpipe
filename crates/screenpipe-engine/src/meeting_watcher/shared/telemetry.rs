...
fn capture_event_nonblocking(event: &'static str, props: serde_json::Value) {
    tracing::debug!(target: "meeting_telemetry", event, props = %props);
}
...
pub(crate) fn capture_detection_decision(
    ...
    capture_event_nonblocking("meeting_detection_decision", props);
}
