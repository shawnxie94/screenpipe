...
use screenpipe_db::BrainSearchHit;
...
pub async fn search(args: BrainSearchArgs<'_>) -> BrainSearchResult {
    ...
    let _ = any_hit;
    let coverage = args.db.brain_search_coverage().await.ok().map(|(indexed, total)| {
...
async fn hit_to_evidence(db: &Arc<DatabaseManager>, hit: &BrainSearchHit) -> EvidenceRefDto {
    let (kind, captured_at, media_available) = if hit.kind == "memory" {
        (SourceKind::Memory, Utc::now(), true)
    } else if let Some(row) = db.brain_get_source(&hit.ref_uid).await.ok().flatten() {
        (
