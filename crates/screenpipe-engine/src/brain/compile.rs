...
use screenpipe_db::{BrainJobKind, DatabaseManager};

use super::executor::{BrainLimits, CompletionRequest};
...
async fn run_compile(ctx: JobContext) -> Result<JobOutcome, JobFailure> {
    ...
    let scope_units = ctx
        .db
        .brain_list_scope_work_units(&scope, 20)
        .await
    ...
    let existing = json!({
        "sop": ctx.db.brain_find_knowledge_by_scope(&scope, "sop").await.ok().flatten().map(|k| k.id),
        "decision_rules": ctx.db.brain_find_knowledge_by_scope(&scope, "decision_rule").await.ok().flatten().map(|k| k.id),
        "exception_playbooks": ctx.db.brain_find_knowledge_by_scope(&scope, "exception_playbook").await.ok().flatten().map(|k| k.id),
    });
...
/// Queue-depth helper for `/brain/status` (candidates awaiting review).
...
#[allow(dead_code)]
fn keep_imports(_: BrainJobKind) {}
