...
use screenpipe_db::{
    compute_input_hash, fingerprint, BrainSourceInput, BrainSourceRegistration, OfficeSourceMeta,
...
pub async fn register_capture_source(
...
) -> Result<screenpipe_db::BrainSourceRegistration, sqlx::Error> {
    ...
    let input = screenpipe_db::BrainSourceInput {
        kind,
        locator,
        revision,
    ...
    db.brain_register_source(&input).await
}
...
pub async fn register_office_source(
    ...
    let registration = db.brain_register_source(&input).await?;
...
pub async fn load_evidence(
    db: &DatabaseManager,
    source_uids: &[String],
) -> Result<Vec<LoadedEvidence>, BrainError> {
    let rows = db
        .brain_get_sources(source_uids)
        .await
        .map_err(|e| BrainError::new("db_error", e.to_string(), true))?;
    let mut out = Vec::with_capacity(rows.len());
    for row in rows {
        let kind = SourceKind::from_str(&row.kind).ok_or_else(|| {
            BrainError::new("invalid_source_kind", format!("kind {}", row.kind), false)
        })?;
        ...
        let (text, media_available, source_url) = match kind {
            ...
            | SourceKind::OfficeSummary => {
                ...
                let text = db
                    .brain_source_text(&row.source_uid, &row.revision)
                    .await
                    .map_err(|e| BrainError::new("db_error", e.to_string(), true))?
                    .map(|(t, _archived)| t);
