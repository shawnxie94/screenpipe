...
//! as brain sources and keeps cursors, statuses and deletion propagation
...
use screenpipe_db::{BrainJobKind, BrainOfficeConnectionRow, BrainOfficeConnectionUpdate, SourceKind, DatabaseManager};
...
impl OfficeService {
    ...
    fn provider(provider: &str) -> Result<OfficeProvider, BrainError> {
        OfficeProvider::from_str(provider)
            .ok_or_else(|| BrainError::new("unknown_provider", format!("未知办公平台 {provider}"), false))
    }
    ...
    pub async fn status(&self, provider: &str) -> Result<OfficeStatusDto, BrainError> {
        let provider = Self::provider(provider)?;
        self.db
            .brain_office_ensure_connection(provider.as_str())
            .await
            .map_err(|e| BrainError::new("db_error", e.to_string(), true))?;
        let row = self
            .db
            .brain_office_get_connection(provider.as_str())
            .await
            .map_err(|e| BrainError::new("db_error", e.to_string(), true))?
            .expect("ensured row");
        let scope = self
            .db
            .brain_office_get_scope(provider.as_str())
            .await
            .map_err(|e| BrainError::new("db_error", e.to_string(), true))?
        ...
        let imported = self
            .db
            .brain_office_imported_object_count(provider.as_str())
            .await
            .map_err(|e| BrainError::new("db_error", e.to_string(), true))?;
        Ok(Self::dto(provider, &row, scope, imported))
    }

    pub async fn status_all(&self) -> Result<Vec<OfficeStatusDto>, BrainError> {
        let mut out = Vec::new();
    ...
    pub async fn refresh(&self, provider: &str) -> Result<OfficeStatusDto, BrainError> {
        let provider = Self::provider(provider)?;
        self.db
            .brain_office_ensure_connection(provider.as_str())
            .await
        ...
        let mut updates = BrainOfficeConnectionUpdate {
        ...
        self.db
            .brain_office_update_connection(provider.as_str(), updates)
            .await
            .map_err(|e| BrainError::new("db_error", e.to_string(), true))?;
    ...
    pub async fn save_scope(
        ...
        if scope.window_end_ms > 0 && scope.window_start_ms > scope.window_end_ms {
            return Err(BrainError::new("scope_invalid", "时间窗口不合法", false));
        }
        for doc in &scope.document_ids {
            if !is_feishu_doc_ref(doc) {
                return Err(BrainError::new("scope_invalid", format!("不是有效的飞书文档: {doc}"), false));
            }
        }
        for chat in &scope.chat_ids {
            if !chat.starts_with("oc_") {
                return Err(BrainError::new("scope_invalid", format!("不是有效的会话 ID: {chat}"), false));
            }
        }
        for meeting in &scope.meeting_ids {
            if meeting.is_empty() {
                return Err(BrainError::new("scope_invalid", "会议 ID 不能为空", false));
            }
        }
        let row = self
            .db
            .brain_office_get_connection(provider)
            .await
            .map_err(|e| BrainError::new("db_error", e.to_string(), true))?;
        if let Some(row) = row {
            if row.scope_revision != expected_revision {
                return Err(BrainError::new("conflict", "范围已被其他人修改，请刷新后重试", false));
            }
        }
        let scope_json = serde_json::to_string(&scope)
            .map_err(|e| BrainError::new("scope_invalid", e.to_string(), false))?;
        let revision = self
            .db
            .brain_office_save_scope(provider, &scope_json)
            .await
            .map_err(|e| BrainError::new("db_error", e.to_string(), true))?;
        ...
        let disabled = self
            .db
            .brain_office_disable_out_of_scope(provider, &account, &keep)
            .await
            .map_err(|e| BrainError::new("db_error", e.to_string(), true))?;
        if !disabled.is_empty() {
            let knowledge_hits = self
                .db
                .brain_invalidate_consumers(&disabled)
                .await
                .map_err(|e| BrainError::new("db_error", e.to_string(), true))?;
            for hit in &knowledge_hits {
                let _ = self
                    .db
                    .brain_set_knowledge_availability(&hit.consumer_id, "stale")
                    .await;
            }
        }
        Ok(revision)
    ...
    pub async fn enqueue_sync(
        ...
        let row = self
            ...
            .await
            .map_err(|e| BrainError::new("db_error", e.to_string(), true))?
            .ok_or_else(|| BrainError::new("unknown_provider", "连接不存在", false))?;
        if row.scope_revision != expected_revision {
            return Err(BrainError::new("conflict", "范围已变化，请刷新后重试", false));
        }
        ...
        let scope = self
            .db
            .brain_office_get_scope(provider)
            .await
            .map_err(|e| BrainError::new("db_error", e.to_string(), true))?
            .map(|(json, _)| json)
            .unwrap_or_default();
        if serde_json::from_str::<OfficeScope>(&scope)
            ...
            .unwrap_or(true)
        {
            return Err(BrainError::new("scope_empty", "请先选择要读取的范围", false));
        }
        self.db
            .brain_office_update_connection(
                provider,
                BrainOfficeConnectionUpdate {
    ...
    /// global CLI login.
    pub async fn disconnect(&self, provider: &str, local_data: &str) -> Result<(), BrainError> {
        Self::provider(provider)?;
        let _ = self
            .db
            .brain_cancel_jobs(Some(BrainJobKind::OfficeSync), None, "disconnected")
            .await;
        if local_data == "erase" {
            self.db
                .brain_office_erase(provider)
                .await
                .map_err(|e| BrainError::new("db_error", e.to_string(), true))?;
        } else {
            // Default: retain inactive copies (sources stay, excluded from Q&A).
            self.db
                .brain_office_disable_out_of_scope(provider, "", &[])
                .await
                .map_err(|e| BrainError::new("db_error", e.to_string(), true))?;
        }
        self.db
...
pub(crate) fn reject_office_provider(id: &str) -> Option<BrainError> {
...
pub fn office_sync_handler(
    ...
    Arc::new(move |ctx: JobContext| {
        ...
        Box::pin(async move {
            let scope_json = ctx
                .db
                .brain_office_get_scope(&ctx.scope_key.clone().unwrap_or_default())
                .await
    ...
    match provider_enum {
        OfficeProvider::Feishu => {
            ...
            for chat_id in &scope.chat_ids {
                ...
                let _ = service
                    .db
                    .brain_office_get_cursor(provider, &cursor_key)
                    .await;
                ...
                loop {
                    ...
                    if page.complete {
                        ...
                        service
                            .db
                            .brain_office_set_cursor(provider, &cursor_key, &end)
                            .await
                            .map_err(|e| OfficeError::new(OfficeErrorCode::ProviderError, e.to_string()))?;
                        break;
                    }
...
/// Register one normalized object as a brain source + office object row.
...
async fn register_object(
    ...
    let _ = service.db.brain_search_upsert_doc(
