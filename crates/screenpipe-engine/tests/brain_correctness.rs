...
// Review-derived Local Brain correctness probes. Synthetic SQLite only.

use std::{collections::HashMap, sync::Arc};

use chrono::Utc;
use screenpipe_db::{
    BrainSearchDocInput, BrainSourceInput, DatabaseManager, DeletionCause, OfficeSourceMeta,
    SourceKind, SourceLocator,
};
use screenpipe_engine::brain::{
    registry::{validate, RegistryContext},
    search::{search, BrainSearchArgs},
...
fn source(office: bool, locator_id: i64, captured_at: chrono::DateTime<Utc>) -> BrainSourceInput {
    BrainSourceInput {
        kind: if office {
...
async fn indexed_source(db: &Arc<DatabaseManager>, office: bool) -> String {
    ...
    let registered = db.brain_register_source(&input).await.unwrap();
    if office {
        db.brain_upsert_office_object(
            "feishu",
            "review-account",
            "message",
            "om_review",
        ...
        .unwrap();
        assert!(db
            .brain_office_get_object("feishu", "review-account", "message", "om_review")
            .await
            .unwrap()
            .is_some());
    }
    db.brain_search_upsert_doc(&BrainSearchDocInput {
...
async fn candidate(db: &Arc<DatabaseManager>, scope: &str) -> String {
    db.brain_create_knowledge_candidate(
        "sop",
        scope,
        "reviewmarker",
...
async fn publish(
    ...
    let version_row = db.brain_get_version(id, version).await.unwrap().unwrap();
    assert!(db
        .brain_publish_version(id, version, &version_row.revision, expected, due)
        .await
        .unwrap());
}
...
async fn indexed_knowledge(db: &Arc<DatabaseManager>, due: &str) -> String {
    ...
    let source = db.brain_get_source(&source_uid).await.unwrap().unwrap();
    let work_unit = db
...
async fn deletion_p01_explicit_erase_must_clear_derived_body_and_index() {
    ...
    let shared = BrainShared::new(db.clone(), temp.path().to_path_buf());
    shared
        .deletion()
...
async fn migration_runtime_p02_journal_ahead_of_db_must_replay_after_restore() {
    ...
    let shared = BrainShared::new(db.clone(), temp.path().to_path_buf());
    std::fs::write(
        shared.brain_dir.join("deletion-journal.jsonl"),
        format!(
            "{}\n",
...
async fn retrieval_p03_disabled_source_must_not_be_retrieved() {
    let db = db().await;
    let uid = indexed_source(&db, false).await;
    db.brain_set_source_state(&[uid], "disabled").await.unwrap();
    let filters = AnswerFilters::default();
    let result = search(BrainSearchArgs {
        db: &db,
...
async fn retrieval_p05_knowledge_must_obey_app_filters() {
    ...
    let matched = search(BrainSearchArgs {
        db: &db,
        question: "reviewmarker",
...
async fn review_publication_p07_second_knowledge_v2_must_supersede_its_v1() {
    ...
    db.brain_add_knowledge_candidate_version(&id, "v2", "{}", "new-input", &[])
        .await
        .unwrap();
    publish(&db, &id, 2, Some(1), "2099-01-01T00:00:00Z").await;
    let version_one = db.brain_get_version(&id, 1).await.unwrap().unwrap();
    assert_eq!(
...
async fn review_publication_p08_failed_publish_cas_must_preserve_current_version() {
    ...
    db.brain_add_knowledge_candidate_version(&id, "v2", "{}", "new-input", &[])
        ...
        .unwrap();
    assert!(!db
        .brain_publish_version(&id, 2, "wrong-revision", Some(1), "2099-01-01T00:00:00Z")
        .await
        .unwrap());
    let version_one = db.brain_get_version(&id, 1).await.unwrap().unwrap();
    assert_eq!(
...
async fn office_control_office_evidence_p09_erased_office_object_must_not_reimport() {
    ...
    shared
        ...
        .unwrap();
    let registration = db
        .brain_register_source(&source(true, 0, Utc::now()))
        .await
        .unwrap();
    assert!(
...
async fn retrieval_answer_p11_positive_control_valid_source_is_retrievable() {
    let db = db().await;
    let uid = indexed_source(&db, false).await;
    let registered = db.brain_get_source(&uid).await.unwrap().unwrap();
    ...
    let source_text = db
        .brain_source_text(&uid, &registered.revision)
        .await
        .unwrap()
...
async fn deletion_concurrent_waves_have_unique_durable_sequences() {
    ...
    let journal = tokio::fs::read_to_string(temp.path().join("brain/deletion-journal.jsonl"))
        .await
        .unwrap();
...
async fn deletion_corrupt_tail_fails_closed_and_keeps_source() {
    ...
    assert!(db.brain_get_source(&uid).await.unwrap().is_some());
}
